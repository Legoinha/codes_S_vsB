...
// MC FILEs
const char * files[] = {
    "Bu_phat5_Bfinder.root",
    "Bd_phat5_Bfinder.root",
...
for (int ifile = 0; ifile < sizeof(files)/sizeof(files[0]); ++ifile) {
    path_to_file = Form("/lstore/cms/henrique/Bmeson/MC_DATA/MC_ppRef_Bmeson/%s", files[ifile]);
    ...
    for (int i = 0; i < nVars; ++i) {
        ...
        if (path_to_file.Contains("Bd")){ 
            ...
            treeMix->Draw(Form("%s >> hist_BKG", var.Data()), Form("!%s && %s && %s", isMCsignal.Data(), cut.Data(), sepcCASES.Data()));  // BKG -- (notice the *!* in the first %s)
        }
